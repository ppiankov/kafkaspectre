...
func newRootCmd() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		PersistentPreRun: func(cmd *cobra.Command, args []string) {
			...
			// WO-25: --format aliases --output. An explicit --output on the
			// subcommand beats a --format on the root, matching cobra precedence.
			if trimmed := strings.TrimSpace(format); trimmed != "" {
				if outFlag := cmd.Flags().Lookup("output"); outFlag != nil && !outFlag.Changed {
					_ = outFlag.Value.Set(trimmed)
				}
			}
		},
	}
	...
	// WO-25: --format is the ANCC-standard JSON output flag. It aliases
	// --output so an agent following the convention gets JSON without learning
	// this tool's flag name. An explicit --output still wins.
	cmd.PersistentFlags().StringVar(&format, "format", "", "Output format (alias for --output: json|sarif|spectrehub|text)")
...
const defaultConfigContents = `# kafkaspectre configuration
# Generated by: kafkaspectre init
#
# Credentials are never read from this file. Set them via environment:
#   export KAFKASPECTRE_USERNAME=your-user
#   export KAFKASPECTRE_PASSWORD=your-password

# Kafka bootstrap server(s), comma-separated
# bootstrap_servers: kafka:9092

# SASL mechanism (PLAIN, SCRAM-SHA-256, SCRAM-SHA-512)
# auth_mechanism: SCRAM-SHA-512

# Default output format (json, sarif, spectrehub, text)
# format: text

# Exclude broker-internal (__-prefixed) topics
# exclude_internal: false

# Exclude topics by name or glob pattern
# exclude_topics:
#   - "__*"
#   - "*.dlq"

# Service-managed backing topics this tool cannot recognise by name
# managed_topics:
#   - "docker-connect-*"
...
func newInitCmd() *cobra.Command {
	return &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			configPath := filepath.Join(".", config.DefaultFileName)
			...
			_, err := fmt.Fprintf(cmd.OutOrStdout(), "Created %s with default configuration.\n", configPath)
			return err
		},
	}
}
