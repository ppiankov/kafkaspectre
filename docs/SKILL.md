...
Kafka topic security and ACL auditor.
...
Scans Kafka clusters for security findings.
...
- Does not remediate or modify Kafka clusters — scan is read-only
- Does not store findings or manage a findings database
- Does not replace dedicated Kafka clusters monitoring — point-in-time security audit only
...
```bash
kafkaspectre scan --output json | jq '.summary'
kafkaspectre scan --output json | jq '.findings[] | select(.severity == "critical")'
```
